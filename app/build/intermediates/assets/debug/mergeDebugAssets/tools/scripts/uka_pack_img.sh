...
echo ".....Sparse image created: \""$pack_d".new.img\"!"
else
echo
echo ".....Image is already sparse."
echo
fi
else
echo
echo ".....No image named "$pack_d.new.img" found in the folder."
echo
fi
}
...
my_pack(){
echo
echo ".....Creating with external file_contexts..."
...
if [ "$size" = "111" ]; then
 ...
 if [ $(echo $?) -eq 0 ]; then
 #echo
 #echo ".....Successfully created "$pack_d".new.img!"
 #echo
 size_output_sparse
 else
 echo
 echo ".....error: AVB structure creation failed!"
 echo
 fi
 else
 echo
...
echo
echo ".....Configuration files are missing in the folder: /data/local/UnpackerSystem/config."
echo
fi
}
...
echo
echo ".....External file_contexts not found, creating from the unpacked image..."
cp ./"$config"/"$pack_d"*_fs_config ./fs_config
...
echo
echo ".....No folder named \""$pack_d"\" for building the image, or the *_pack_sparse.sh file is missing."
echo
fi
#unset erof
busybox rm -f ./file_contexts ./fs_config
