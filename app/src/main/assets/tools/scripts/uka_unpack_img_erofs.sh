...
if busybox test -s "$dir_avb"/"$r_name"/"$r_name"_avb.img; then
 ...
 echo
 echo ".....Error while creating configuration file!"
 echo
 fi
 ...
 echo
 echo ".....Error while detecting AVB!"
 echo
 fi
 return
}
...
if busybox test -s "$config"/"$r_name"*_space.txt; then
 ...
 echo
 echo "...The image "${file}" contains files with spaces, correcting..."
...
#echo "...Corrected, saved in "$config"/"$r_name"_space_contexts.txt"
echo
else
echo
echo "...Error correcting files with spaces!"
echo
fi
else
 echo
...
echo
echo -e "\033[31;47;1m ...error: Error! Invalid file extraction
 The folder /data/local/UnpackerSystem/erofs/$r_name will be unavailable for building!             \033[0m"
echo
